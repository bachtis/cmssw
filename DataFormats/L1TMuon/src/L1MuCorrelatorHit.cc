...
L1MuCorrelatorHit::L1MuCorrelatorHit(int etaRegion,int phiRegion,int depthRegion,uint tfLayer,int globalPhi,int phi,int globalBend,int phiB,int id,int bx,int quality,int eta,int etaQuality,int type):
  etaRegion_(etaRegion),
  phiRegion_(phiRegion),
  depthRegion_(depthRegion),
  tfLayer_(tfLayer),
  globalPhi_(globalPhi),
  phi_(phi),
  globalBend_(globalBend),
  phiB_(phiB),
  id_(id),
  quality_(quality),
  bxNum_(bx),
...
bool L1MuCorrelatorHit::operator==(const L1MuCorrelatorHit& id) const {
  ...
  if ( globalPhi_             != id.globalPhi_ )               return false;
...
ostream& operator<<(ostream& s, const L1MuCorrelatorHit& id) {
  ...
  << "global phi: "        << setw(5) << id.globalPhi_  << " "
  << "phi: "               << setw(5) << id.phi_  << " "
